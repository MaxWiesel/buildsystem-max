...
define PYTHON_STREAMLINK_INSTALL_CONFIG
	$(INSTALL_DATA) -D $(PKG_FILES_DIR)/config $(TARGET_DIR)/.config/streamlink/config
endef
PYTHON_STREAMLINK_TARGET_FINALIZE_HOOKS += PYTHON_STREAMLINK_INSTALL_CONFIG

define PYTHON_STREAMLINK_TARGET_CLEANUP
	rm -rf $(addprefix $(TARGET_SHARE_DIR)/,man zsh)
endef
PYTHON_STREAMLINK_TARGET_FINALIZE_HOOKS += PYTHON_STREAMLINK_TARGET_CLEANUP

$(D)/python-streamlink: | bootstrap
	$(call python-package)
